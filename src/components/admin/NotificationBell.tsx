...
export function NotificationBell() {
  ...
  // Poll every 30s only when tab is visible + refresh on focus
  useEffect(() => {
    let cancelled = false;
    let interval: ReturnType<typeof setInterval> | null = null;

    const load = async () => {
      if (!cancelled && !document.hidden) await fetchNotifications();
    };
  ...
  const handleClick = (n: Notification) => {
    ...
    if (n.notification.linkUrl) {
      router.push(n.notification.linkUrl);
    }
    setOpen(false);
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute right-0 top-11 w-80 max-h-96 bg-card rounded-brand shadow-xl border border-primary/10 z-[100] overflow-hidden flex flex-col">
          {/* Header */}
